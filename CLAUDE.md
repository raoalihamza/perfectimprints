...
**Monthly auto-rebuild (M6-606, IMPLEMENTED):** [.github/workflows/monthly-rebuild.yml](.github/workflows/monthly-rebuild.yml) runs on the 1st of every month at 00:00 UTC (`cron: '0 0 1 * *'`) + `workflow_dispatch`. The dispatch form takes a `phases` input (default `A,B,C,E`) so a subset can be tested without the long Phase C — e.g. run `A,B,E` to validate the pipeline in minutes.
...
- **`assemble`** — downloads all scrape artifacts over the committed copies, then: (1) **AI content for NEW categories only** via `generate_content.py --skip-existing` (`DEEPSEEK_API_KEY`) — existing pages are untouched; since the PI URL set is frozen this is usually a cheap no-op, present as a safety net; (2) **prune removed products** ([scripts/monthly/prune-removed-skus.ts](scripts/monthly/prune-removed-skus.ts)) — drops SKUs no longer in `products.json` from each baked `data/categories/*.json` `productSkus[]` (render-time `resolveProducts` already skips missing SKUs, so pages are correct either way; the prune keeps the committed data accurate + gives an explicit diff); (3) **compute the change summary** ([scripts/monthly/compute-summary.ts](scripts/monthly/compute-summary.ts)) — diffs the fresh data vs `HEAD` for products added/removed/price-changed, new brands + logo changes, and new/updated category pages, writing `pr-body.md` + `summary.json`; (4) **open a review PR** to `main` (branch `monthly-rebuild`, `peter-evans/create-pull-request`); (5) **email Patrick** the summary via Gmail SMTP ([scripts/monthly/send-summary-email.ts](scripts/monthly/send-summary-email.ts)). Steps (4)+(5) only fire when something actually changed (`steps.summary.outputs.changed`).

**Reviewable, not force-merged.** A full-catalog change is intentionally **NOT** auto-merged (unlike the small weekly feeds) — the PR opens for Patrick to review the data diff; CI runs on it before merge. **Phase D is not re-run** (mapping is stable; the committed `pi-to-geiger.json` is used as-is). **Warmup is automatic** — on merge Vercel rebuilds production and emits a successful `deployment_status`, which triggers [.github/workflows/post-deploy-warmup.yml](.github/workflows/post-deploy-warmup.yml); no explicit warmup trigger is needed in the monthly job.

**Required repo secrets** (Settings → Secrets and variables → Actions — on BOTH the staging repo `raoalihamza/perfectimprints` and Patrick's production repo `pbnj53/perfectimprints`): `DEEPSEEK_API_KEY` (AI content for new categories), `GMAIL_USER` + `GMAIL_APP_PASSWORD` (summary email). Optional `LEAD_EMAIL_TO` / `LEAD_EMAIL_FROM` (default `patrick@perfectimprints.com`). **No Sanity secrets** — content writes only to `data/categories/*.json`. Actions settings must already allow Read/write permissions + "Allow GitHub Actions to create and approve pull requests" (same as the weekly scrape jobs). The AI-content + email steps no-op with a logged warning if their secret is absent, so a misconfigured run still produces the data PR.

Manual rebuild trigger lives in Sanity Studio as a custom action on globalSettings (pending — not part of M6-606's workflow).
