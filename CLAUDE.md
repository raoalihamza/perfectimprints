...
- The site-search index is generated at build time by the **`prebuild`** script (`pnpm build:search-index` → `public/search-index.json`); it runs automatically before `next build` when you build via `pnpm build` (which is the Vercel build command). Running `next build` directly skips it and ships a stale index. Search is client-side only — never call a search service (Searchspring or otherwise) at runtime. See Section 11-adjacent M5-502 and `scripts/search-index/README.md`.
