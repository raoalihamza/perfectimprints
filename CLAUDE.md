...
**Searchable CategoryPicker + `category-list.json`.** The 22,180 category pages are build-time JSON, not Sanity docs, so a normal reference field can't target them. [sanity/components/CategoryPicker.tsx](sanity/components/CategoryPicker.tsx) (used by `productPlacement.addToCategories`/`removeFromCategories`) fetches the build-time artifact `public/category-list.json` (slug + title for all 22,180, derived from `data/pi-urls/category-urls.json` by `pnpm build:category-list`, run in `prebuild`) **plus** live `customCategory` slugs from Sanity, filters client-side with debounce, and supports **create-new** (creates a `customCategory` at that slug, which then renders at `/cat/<slug>`). Studio-only — `category-list.json` is never loaded by the live site. All custom Studio components use plain React + the `sanity` form API (no `@sanity/ui` import, which isn't top-level resolvable for the app's typecheck).
...
**globalSettings.** Singleton. Fields: phone number (default `800-773-9472`), contact email, social links, footer columns content, copyright text, CTA banner text, plus per-aggregator curation objects:
