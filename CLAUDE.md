...
- Organization (sitewide, in root layout — name/url/logo/telephone + `contactPoint`; `sameAs`/`address` omitted until real socials/address confirmed) — `organizationSchema()` in [lib/seo/schema-generators.ts](lib/seo/schema-generators.ts)
