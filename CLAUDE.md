...
**Revalidation webhook (manual setup, per environment).** Instant freshness for Sanity-managed content (mega menu, global settings, home, services pages, blogs, videos, custom products/categories, and the live search delta) depends on a single GROQ-powered Sanity webhook POSTing to `/api/sanity/revalidate`. This is a **one-time manual setup in the Sanity project** (API → Webhooks), not provisioned by code — **as of 2026-06-21 it was not yet created** (`0 of 2` webhooks). Create one per environment (staging now, production at launch) with a `SANITY_WEBHOOK_SECRET` matching the Vercel env var. Full steps: [docs/sanity-webhook-setup.md](docs/sanity-webhook-setup.md). Without it, content still updates via ISR/on-demand fallback (up to the route's `revalidate` interval), just not instantly.
