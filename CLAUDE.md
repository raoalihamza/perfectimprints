...
- Custom top-level pages: `/[slug]` (Issue 2 — any published `page` doc with a non-reserved slug renders at its own top-level address via [app/[slug]/page.tsx](app/[slug]/page.tsx), for recreating old-site URLs like `/llm-info-perfect-imprints`; reserved slugs — all existing routes + `api`/`admin` + the 8 static slugs — are blocked at the schema and route level)
- Videos index: `/videos`
...
**Post-deploy warmup (monthly + manual only — NOT per-deploy).** Cold facets serve at 400-800ms on first hit while the on-demand SSG renders; warm ones serve under 200ms from edge cache. Warmup pre-hits all 21,139 facet + compound-facet URLs at concurrency 10 (~30-60min) via `scripts/warmup/warmup-facets.ts`. **It deliberately does NOT run on every deploy.** It used to fire on `deployment_status: success` for every Vercel deploy, but with several deploys a week (the weekly deals/new/rush scrape PRs + ordinary commits — none of which change facet pages) that generated millions of edge requests + ISR writes and **exhausted Vercel's account-wide free-tier limits**. A full 21K warmup is only actually needed after the **monthly full-catalog rebuild**, the one event that turns every facet page cold. So warmup now runs only: (1) **automatically after the monthly rebuild's production deploy** — [.github/workflows/monthly-warmup.yml](.github/workflows/monthly-warmup.yml) listens to `deployment_status`, runs a cheap `gate` job that exits in seconds for every deploy EXCEPT the `Production` deploy whose merge commit is `chore(data): monthly catalog rebuild`, and only then calls the reusable [.github/workflows/post-deploy-warmup.yml](.github/workflows/post-deploy-warmup.yml) (`workflow_call`) targeting production `www`; and (2) **manually** via `post-deploy-warmup.yml`'s `workflow_dispatch` (defaults to staging; pass production www to warm prod) — locally `pnpm warmup` runs the same script and reads `NEXT_PUBLIC_SITE_URL`. Weekly scrape deploys and normal commits **never** trigger the 21K warmup. This is safe because on-demand SSG works without warmup — a cold facet just serves its first hit slower then caches permanently until the next deploy — so dropping per-deploy warming costs only a little long-tail speed right after small deploys while removing the recurring resource drain. The monthly rebuild PR (M6-606) is still covered automatically once merged + redeployed; no explicit warmup step lives in `monthly-rebuild.yml`.
...
**Job structure (the 6-hour-per-job split).** GitHub-hosted runners cap a single job at 6h, and Phase C alone is ~6h at 1 req/sec, so the run is split into separate jobs joined by artifacts (no single job runs A+B+C):

- **`config`** — parses the `phases` input into `run_ab` / `run_c` / `run_e` flags (scheduled runs = all).
- **`scrape-ab`** — Phase A → B (taxonomy + product catalog), ~25-45 min. Checkpoint-cached (`phase-b-<run_id>` with `phase-b-` restore-key) + `--resume`. Uploads `categories.json` + `products.json` as the `geiger-ab` artifact.
- **`scrape-e`** — Phase E (brand logos), ~5 min. Re-runs the cheap Phase A so it's independent of `scrape-ab` and runs in parallel. Uploads `brands.json` + `brand-logos/` + `public/brand-logos/` as `geiger-e`.
- **`scrape-c`** — Phase C (facet/modifier memberships), the long pole. Runs with `--workers 6` (six independent 1-req/sec clients per §16 → ~1h wall-clock, comfortably under the 6h cap) **plus** `--resume` and a cached `phase-c-<run_id>` checkpoint, so an overrun resumes from the last 100-URL checkpoint on a re-run instead of restarting. `timeout-minutes: 350`. Downloads `geiger-ab` when present (else uses the committed `products.json`/`categories.json`); uses the committed `pi-to-geiger.json`. Uploads `facet-memberships.json` as `geiger-c`.
- **`assemble`** — downloads all scrape artifacts over the committed copies, then: (1) **AI content for NEW categories only** via `generate_content.py --skip-existing` (`DEEPSEEK_API_KEY`) — existing pages are untouched; since the PI URL set is frozen this is usually a cheap no-op, present as a safety net; (2) **prune removed products** ([scripts/monthly/prune-removed-skus.ts](scripts/monthly/prune-removed-skus.ts)) — drops SKUs no longer in `products.json` from each baked `data/categories/*.json` `productSkus[]` (render-time `resolveProducts` already skips missing SKUs, so pages are correct either way; the prune keeps the committed data accurate + gives an explicit diff); (3) **compute the change summary** ([scripts/monthly/compute-summary.ts](scripts/monthly/compute-summary.ts)) — diffs the fresh data vs `HEAD` for products added/removed/price-changed, new brands + logo changes, and new/updated category pages, writing `pr-body.md` + `summary.json`; (4) **open a PR** to `main` (branch `monthly-rebuild`, `peter-evans/create-pull-request`); (5) **email Patrick** the summary via Gmail SMTP ([scripts/monthly/send-summary-email.ts](scripts/monthly/send-summary-email.ts)); (6) **auto-merge the PR** as the FINAL step (`gh pr merge --squash --delete-branch`). Steps (4)–(6) only fire when something actually changed (`steps.summary.outputs.changed`).
