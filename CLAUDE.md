...
Both lists are built in `prebuild` and written to **BOTH `public/` (served by the embedded Studio at `/admin`) and `sanity/static/` (served by the standalone `sanity dev` Studio, which does NOT serve Next's `public/`)** — the shared loader [sanity/components/load-json.ts](sanity/components/load-json.ts) (`loadStudioJson`) tries `/<file>` then `/static/<file>`, requires a JSON content-type (rejects SPA index.html fallbacks), and surfaces a clear "couldn't load" message instead of a misleading "no match" when neither resolves. `sanity/static/` is git-ignored (regenerated by `prebuild`). Studio-only — these lists are never loaded by the live site.
