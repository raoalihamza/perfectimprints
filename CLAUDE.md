...
- Organization (sitewide, in root layout — name/url/logo/telephone + `contactPoint`; **`sameAs` = the URLs of the ENABLED `globalSettings.socialLinks` only, `telephone`/`email`/`PostalAddress` from `globalSettings.contact`**; `sameAs`/`address` are omitted when empty rather than fabricated) — `organizationSchema(settings)` in [lib/seo/schema-generators.ts](lib/seo/schema-generators.ts), fed by `getSiteSettings()` via the async [components/seo/OrganizationJsonLd.tsx](components/seo/OrganizationJsonLd.tsx)
