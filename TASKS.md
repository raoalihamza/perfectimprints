...
### [ ] P2-AI-005 (part 2 of 2): landing-specific per-page lead form — PENDING

- Quantity Needed + Date Needed required; per-page recipient from `landingPage.leadRecipient` (field already stored); lead record + email to the recipient + automatic customer confirmation email. Replaces the Phase 1 `LeadForm` stand-in at the template's `#quote-form` slot (marked with a `P2-AI-005 part 2` comment in [components/landing/LandingPageTemplate.tsx](components/landing/LandingPageTemplate.tsx)).
- After part 2 + Patrick's sign-off on the city×product combos: produce the top 10 priority pages as content with the generator (by-city list: Sylva, Asheville, Waynesville, Bryson City, Franklin NC; Fort Walton Beach, Destin, Navarre, Crestview, Miramar Beach FL; by-topic: screen printed t-shirts, company uniforms, etc.), checking each is genuinely unique.
