...
### [x] Q-200: Patrick's two confirmed changes (rep phone default, full customer block) - CODE COMPLETE 2026-08-07 (staged, NOT committed)

Patrick reviewed the finished Quick Quote module and answered the open questions. **Four things he confirmed as correct stay exactly as built and were not touched:** numbering starting at `Q-1001`, the Geiger item number staying hidden from the customer (deliberate - it stops the customer price shopping the quote against the catalogue), pricing pulled from the PUBLISHED product rather than a draft, and the thirty day expiry default. Two things change. One deploy.

**Change 1 - the rep phone on new quotes.** The default was `850-200-4020`. It is now the main company line `800-773-9472`, the number in the site header.

- **It was never hardcoded.** [sanity/schemas/documents/quote.ts](sanity/schemas/documents/quote.ts) resolves it from Global Settings, and the live singleton holds `contact.phones = ["850-200-4020", "828-214-7681"]` with the legacy flat `phoneNumber = "800-773-9472"`. The old code read `contact.phones[0]` first, so it picked up a direct line. **The fix is the ORDER, in that one expression:** `phoneNumber` (the single MAIN published line) before `contact.phones[0]` (the ordered footer LIST), with a named `FALLBACK_REP_PHONE` constant last for when settings cannot be read at all. No second source of truth, no new field.
- **Deliberately NOT fixed by reordering `contact.phones`.** That list also drives the footer's phone order and the Organization JSON-LD `telephone`, so moving it would have changed two unrelated public surfaces to fix a quote default. It is also Patrick's content, not ours to rewrite.
- **No existing quote is modified.** An `initialValue` runs once, at document creation. The two published quotes in the dataset keep what they were given (`Q-1002` carries `850-200-4020`, `Q-1003` carries `850-290-5534`); Patrick can change either by hand in Studio if he wants to. The verification script records every non-fixture quote's `repPhone` before the run and compares after, and reports any that still carry an older number rather than touching them.

**Change 2 - the customer's full details on the quote.** Their email, phone and address were withheld in Q-140, reasoning that a forwarded quote should carry less of their information. Patrick decided he wants the full block. Implemented on **both** the web page ([components/quote/QuoteDocument.tsx](components/quote/QuoteDocument.tsx)) and the **PDF** ([lib/quotes/quote-pdf-model.ts](lib/quotes/quote-pdf-model.ts) + [lib/quotes/pdf/QuotePdfDocument.tsx](lib/quotes/pdf/QuotePdfDocument.tsx)).

- **No new read.** The GROQ projection in [lib/sanity/queries/quotes.ts](lib/sanity/queries/quotes.ts) already fetched `customerEmail`/`customerPhone`/`customerAddress` into `quote.customer`; the page simply did not render them and the PDF model did not type them. The page still makes exactly ONE Sanity read.
- **One rule for the address, imported by both surfaces.** The new pure `quoteAddressLines` in [lib/quotes/quote-display.ts](lib/quotes/quote-display.ts) (7 vitest cases) splits on newlines, trims, collapses inner runs of spaces, **drops blank lines** so a pasted address opens no gap in the middle, and caps at `QUOTE_ADDRESS_MAX_LINES` = 8 so one pathological paste cannot push the line-item table around. Two independent splitters would have drifted the first time either was touched.
- **Missing values render as nothing.** Only the email is REQUIRED on a quote, so blank phone and address are the ordinary case, not the edge: each field renders only when it has a value, and the "Your custom quotation" fallback now fires only when the WHOLE block is empty. No empty label, no stray comma, no `undefined`.
- **Tappable, matching the rep block** - `mailto:` on the email and `tel:` on the phone, and `break-words` on the email so a long address does not widen the card on a phone.
- **The PDF layout constraint was the real risk and is handled.** The party boxes are already `flexBasis: 0`, which is what makes a long address wrap inside its own column instead of widening it and shoving the other box sideways (the spike's exact failure, in a different place). The address prints one `<Text>` per line rather than one blob, so the renderer wraps within the box. The verification script asserts every text run still starts inside the page margins, on the full fixture and the minimal one.
- **Nothing else moved.** Still withheld on BOTH surfaces: the internal `title`, `sentAt`, the Geiger line `sku`, and any `/products/<slug>` link. Asserted on the page HTML, in the extracted PDF text, and structurally in the PDF source.

**Deliberately out of scope:** scoping the blog and video index search boxes to their own content type. Patrick has asked for it, it is agreed for a later round, and the site-wide behaviour including Q-180's priority grouping is untouched.

**Verification:** [scripts/quick-quote/verify-q160.ts](scripts/quick-quote/verify-q160.ts) was EXTENDED rather than forked - it already drives exactly what changed, end to end. Same safety rules: `zz-test-` ids with a guard re-checked at deletion, the counter recorded and restored exactly, cleanup in a `finally` that survives a crash, `.invalid` test emails that cannot reach a real inbox, and the preflight that refuses to run against a deployment without the route. New: a THIRD fixture (`zz-test-quote-q200-minimal`) carrying only the required email, because "renders cleanly when the optional fields are blank" is the case a full fixture can never prove; the empty-field checks are scoped to the customer card rather than the whole page, since a healthy page legitimately contains nulls in its React payload. **Dry run: 78 checks, all pass, zero failures** (offline source checks, a local render of the full awkward fixture, and a local render of the minimal one). Note that running it regenerated [docs/quick-quote/Q-160-verification-report.md](docs/quick-quote/Q-160-verification-report.md), so the committed report currently describes the DRY RUN; the `--apply` run at deploy gate (4) replaces it with the deployed results. The previous apply-mode report is in git history. `pnpm typecheck` clean; 148 quote unit tests pass (6 files), 243 across the whole suite (14 files), zero failures.

- **No webhook Filter change:** no new document type. `quote` and its Filter entry were wired in Q-110, and this task adds no read surface. No new cache tag, env var, or dependency.
- **`/quote/[token]` stays static:** no `searchParams`, no uncached read, and the action island still takes every value as a prop and never calls `useSearchParams`. The customer block is server-rendered markup.
- **Deploy gates (Ali, ONE deploy):** (1) create a quote in Studio and confirm the rep phone defaults to `800-773-9472`; (2) open a quote link whose customer has a full address and check the block reads well, on a phone too; (3) download that quote's PDF and confirm the address block sits properly with nothing pushed off the page; (4) run `pnpm tsx scripts/quick-quote/verify-q160.ts --apply`.

### [x] FIX-830: four outstanding no-charge commitments on one deploy - CODE COMPLETE 2026-08-14 (staged, NOT committed)

Four small pieces batched because each shares the same deploy. Every one was either promised to Patrick in writing or found while investigating something he reported. Applied to **all three repos** (local, production `patrick-perfectimprints/perfectimprints`, staging `staging-perfectimprints`); the production repo was read as the source of truth for what is live. Nothing committed, nothing pushed, no dashboard or webhook work.

**Two standing corrections recorded here because a previous session got them backwards.** (1) **The Geiger images are usable.** Patrick: *"With a majority of the images, they come from the individual suppliers. Distributors like me and Geiger have permission to use them on our side or for presentations."* An earlier session concluded the opposite and was wrong. He has separately DECLINED `acquireLicensePage`: *"I don't want people approaching me about licensing images."* Usage terms yes, an invitation to license from him no. (2) **The rebuild pipeline is not broken.** Every Site Refresh action opens its own PR which auto-merges and deploys; `products.json` is old only because the hours-long Full Catalog Rebuild has never been run.

**Task 1 - the product fields Google is asking for.** Full write-up in CLAUDE.md Section 7 (productPage, "Product structured data"). Rules in the new pure [lib/products/product-schema.ts](lib/products/product-schema.ts) (16 vitest cases), wired in [app/products/[slug]/page.tsx](app/products/[slug]/page.tsx).

- **Audited before changing anything:** all 78 `/products/` URLs in the live sitemap were fetched and their Product JSON-LD parsed. Description 78/78, image 78/78, offers 78/78, **`sku` 0/78**, availability 0/78, return policy 0/78. Against Sanity: 86 published productPages, **86/86 carry an item number**, 86/86 have at least one valid pricing tier, 86/86 have a description, 86/86 have a FOB zip. So the identifier was on every document and was simply never projected into the schema - that one is a plain bug, not a data gap.
- **Price = one full minimum order, including setup** (Patrick's decision), always carrying the quantity it buys via `eligibleQuantity` AND a `UnitPriceSpecification.referenceQuantity`. The figure is computed by the CONFIGURATOR's own `estimateForQuantity` / `effectiveSetupCharge` / `decorationUpchargeFor` with the page's own defaults, so it is the Estimated total the visitor sees, not a private calculation - proved on `/products/1785-illini`: 80 x $6.06 + $45 = **$529.80** in the markup and `$529.80` in the rendered page text. No tier, no offer.
- **Return policy:** `MerchantReturnNotPermitted` + link to `/returns`. The 14-day window and restocking fee are BLANK-goods terms and are deliberately not stated from a product page - overstating a returns promise on custom printed work is a real commercial risk.
- **Availability** is a new editable Studio dropdown (default "Available to order"), not an assertion invented in code, and a non-default value renders on the page too. **Item condition** is `NewCondition`. **Schema images forced to jpg** (AVIF is not a Google-supported structured-data image format and 2 live pages were serving it).
- **Reported, not fabricated:** shipping (Google wants a published rate + destination + delivery time; the business quotes shipping - Patrick's call whether to publish a flat rate), and `gtin`/`mpn` (no GTIN exists on promotional blanks; `mpn` only if Patrick confirms these are supplier part numbers).
- **Product Snippets protection:** the report shows 43 valid / 0 invalid, so the replacement `Offer` keeps `price` + `priceCurrency` + `url` and adds `availability` - at least as strong as the `AggregateOffer` it replaced. Validated with 0 errors against `validator.schema.org`.

**Task 2 - video structured data, and why only 2 of 73 were valid.** **The markup was not the cause.** All 71 published video pages were fetched live and checked field by field: `name` / `description` / `thumbnailUrl` / `uploadDate` present on **71/71**, 0 JSON parse errors, all 71 thumbnails returning 200 with real image bytes, the player `<iframe>` in the static HTML, no `noindex`, self-canonical, all 71 in the sitemap, `robots.txt` clean, and `customSchema` holds exactly one doc (a `WebApplication` on `/blog`) so nothing else emits a VideoObject. Identical complete markup on all 71 cannot explain 2 valid, so the gap is crawl and verification coverage, not structured data. **What WAS genuinely wrong, on 71 of 71: `contentUrl` pointed at the YouTube watch page**, which Google's reference explicitly forbids and which Google fetches to verify the video. It now passes through `isDirectMediaUrl()` and is omitted for anything that is not a media file; `url` (the page canonical) was added. **`duration` stays absent** - nothing records it and inventing one is worse than omitting it. **Page-builder and blog-body embeds still emit nothing, correctly** - neither has an upload date, so a VideoObject there would have to be fabricated.

**Task 3 - the search box wording.** `/blog` (and `/blog/page/N`) reads **Search Blogs**, `/videos` reads **Search Videos**, derived from `priorityType` in [components/layout/IndexHeadingWithSearch.tsx](components/layout/IndexHeadingWithSearch.tsx) so page 1 and page N cannot drift. **Wording only** - what is searched is untouched, and scoping stays the separate agreed work. The header box is unchanged (verified in rendered HTML: header and drawer keep the original placeholder). One additive `label` prop on `SearchBox` gives screen readers the same wording.

**Task 4 - the escaping fault.** **12** JSON-LD emitters exist; **4 escaped `<` and 8 did not** (blog post, FAQ library, root-layout WebSite, video detail, Breadcrumbs, page FaqAccordion, OrganizationJsonLd, and `components/seo/Schema.tsx` - which is what all 22,180 `/cat` pages render CollectionPage/ItemList/FAQPage through). All 12 now import `jsonLdHtml()` from the new [lib/seo/json-ld.ts](lib/seo/json-ld.ts), which is the SAME escape the four correct sites already used, lifted into one place rather than reinvented. **Output is unchanged for normal content, shown not claimed:** `/cat/water-bottles` was rendered locally on the new code and all three of its JSON-LD blocks are byte-identical to the live page's.

**Deliberately NOT done:** the paid Product Snippets work on ~14,000 category pages (separate engagement); `acquireLicensePage` (Patrick declined); image license metadata (on hold, quote separately); any change to the quote module, the category CTA rule, the capped-category allowlist, the lead pipeline, or the Q-175 freshness work; blog and video search scoping.

**No webhook, dashboard or environment work:** no new Sanity document type (the one new field, `productPage.availability`, sits on a type already in the webhook Filter with an unchanged Projection), no new cache tag, no new env var, no new dependency. Nothing was made dynamic - the only touched render paths gained pure server-side computation, and no client component under any route gained `useSearchParams`.

**Verification:** `pnpm typecheck` clean. **265 tests pass across 16 files** (was 243 across 14; +16 `product-schema`, +6 `json-ld`), zero failures. A local dev render of `/products/1785-illini`, `/videos/premium-branded-gifts-for-national-doctors-day`, `/cat/water-bottles`, `/blog`, `/blog/page/2` and `/videos` drove the end-to-end checks above, and **both the Product and the VideoObject blocks validate with 0 errors against `validator.schema.org`**, Google's own validator. No local production build was run.

**Deploy gates (Ali, ONE deploy):** (1) open a product page and confirm the structured-data price matches the on-page Estimated total and reads as a full minimum order; (2) open a video page and confirm the video data validates; (3) open `/blog` and `/videos` and check the search box wording, on a phone too.

### [x] FIX-810: `/cat/pens` empty grid + unfindable category in the Studio picker - CODE COMPLETE 2026-08-10 (staged, NOT committed)

Patrick reported two things that turned out to be independent bugs sharing one visible symptom: `https://www.perfectimprints.com/cat/pens` showed the lead form instead of products, and *"I tried adding products on the category override but the category won't show in the search."* Both are ours; neither is a data gap. Applied to **all three repos** (local `raoalihamza/perfectimprints`, production `patrick-perfectimprints/perfectimprints`, and `patrick-perfectimprints/staging-perfectimprints`) - the picker sources were byte-identical across all three before the change, verified against `git show HEAD:` rather than assumed.

**Bug 1 - the grid was hidden, not missing.** `data/categories/pens.json` held 60 valid pen SKUs the whole time. `shouldShowEmptyStateCTA` ([lib/categories.ts](lib/categories.ts)) hides the grid whenever `skuFilterMode === 'full-capped-60'`, and pens landed in that mode because the AI pipeline's slug-token filter scores a candidate by asking *"does the product NAME contain the category slug?"* - and real pens are called `Bic Clic Stic`, `Javalina`, `Sharpie`. Almost nothing scored, fewer than 30 survived, the pipeline fell back to an arbitrary top-60, and the CTA rule then (correctly, given what it knew) treated that set as untrustworthy. **The rule is right; its input was wrong.**

- **Deliberately NOT fixed by relaxing the rule.** An audit of all **66** `full-capped-60` roots (all 66 are root-type - the slug-filter tier only runs on root mappings, so the 21,137 facets were never at risk) found **58 are mapped to a broad parent department rather than their own product type**: `desk-organizers`, `laser-pointers`, `letter-openers`, `memo-boards`, `notepads`, `staple-removers` and `stock-shaped-notepads` **all** point at `Home > Office & Technology` (1,738 items). Dropping the rule renders the same 60 unrelated office products across seven pages - precisely the off-topic-grid complaint the rule exists to prevent. Same pattern for `binoculars`/`flying-discs`/`jump-ropes`/`seat-cushions` (all `Sports & Outdoor`) and `hot-sauce`/`pizza-cutters`/`measuring-cups` (all `Drinkware`). Those 58 have a **mapping** problem and are untouched - separate, unquoted work.
- **Fixed in the DATA, for 7 allowlisted slugs only**, by the new [scripts/monthly/expand-safe-capped-categories.ts](scripts/monthly/expand-safe-capped-categories.ts) (`pnpm monthly:expand-safe`, `--dry-run` supported): `pens` 60 to 365, `office` 60 to 1738, `bags` 60 to 1015, `health` 60 to 617, `medical-healthcare-items` 60 to 362, `calendars` 60 to 87, `workwear` 29 to 29 (mode-only). Each is a slug whose Geiger path genuinely IS its product type, or - for `office`/`bags`/`health` - whose PI slug IS that department. The allowlist is hard-coded with a per-slug reason; **do not extend it without re-running the mapping audit for that slug**.
- **The derivation was validated, not trusted.** It reproduces the pipeline's own `full` tier (every product whose `category_paths` includes the mapped path or a descendant) and was checked against each target's recorded `rawSkuCount` **before** anything was written: 365/87/29/362/1738/1015/617, seven for seven. A spot-check of 15 SKUs spread across the new pens list returned 15 real pens.
- **It re-derives on every run.** A first draft early-exited when `skuFilterMode === 'full'`; that guard was removed because it made the re-run-after-a-Full-Catalog-Rebuild path a silent no-op *precisely* when a fresh catalog has new products for these pages - i.e. the script's own docstring would have been a lie. It now reports `no change` when input and output match, which is still idempotent (verified: second run wrote nothing, diff unchanged).
- **Diff hygiene:** exactly 7 files touched, only `productSkus` / `skuFilterMode` / `rawSkuCount` / `filteredSkuCount` changed, and the writer preserves each file's existing CRLF line endings so the diff shows only real changes.

**Bug 2 - the picker could not surface the category.** [sanity/components/CategoryPicker.tsx](sanity/components/CategoryPicker.tsx) and [sanity/components/ProductPicker.tsx](sanity/components/ProductPicker.tsx) both walked their list in file order and stopped at the first 30 substring matches. Typing `pens` matched **246** entries; the first 30 were `apparel/supplier/hubpens`, `carpenter-pencils/supplier/hubpens` and **28 `dispensary-containers/*` rows** (dis-**pens**-ary). Exact `pens` sat at **position 50** and never rendered. Confirmed against live Sanity: no `pens` override doc exists, only an unrelated `pens/feature/nfc-enabled` - Patrick genuinely could not create one. **Not pens-specific:** any short slug inside longer ones (`bags`, `hats`, `mugs`, `pins`, `caps`) behaves the same, and ProductPicker had the identical loop, so an exact SKU or brand could be buried too.

- Both now score **every** match and keep the best 30 via the shared pure [sanity/components/rank-matches.ts](sanity/components/rank-matches.ts). Tiers: exact-primary, exact-label, primary-starts-with-on-a-boundary, label-whole-word, primary-whole-word, plain-substring; ties break by slug depth (a root outranks its own facet children), then length, then alphabetically, so the same query always renders the same list. A "boundary" is a non-alphanumeric character or the string edge - that alone separates `gel-pens` from `dispensary`, with no word list to maintain.
- **No index needed and none added:** scoring ~22k entries costs a few ms and runs behind the pickers' existing 180 ms debounce.
- Both dropdowns gained a **"Showing the 30 closest of 246 matches"** footer. The silent truncation is what made the old picker read as *"the category isn't there"* rather than *"it's further down"*.
- **Verified against the real 22,180-entry `category-list.json`**, not a fixture: `pens` to #1, `bags` to #1, `mugs` to `cafe-mugs`/`coffee-mugs` (no exact `mugs` category exists), and product search `bic` to BIC pens first.
- `rank-matches.ts` is Studio-only and dependency-free - the standalone Studio bundler cannot import the app's `lib/`.

**Also established while diagnosing (facts, not changes):**

- **Overrides survive a refresh.** Patrick's worry that a refresh re-adds SKUs he removed is unfounded: the entire refresh pipeline contains **zero** Sanity references (`scripts/monthly/` plus the workflow), and `hiddenSkus` is applied at RENDER time over the baked list in `mergeCategoryProducts`, so a returning SKU is filtered on every page load. The real asymmetric risk is the opposite one: an **added** SKU that Geiger discontinues resolves to nothing and vanishes silently, which matters most on the **121 of 154** overrides that run `replaceProducts`.
- **The full catalog has never been refreshed.** `data/geiger/products.json` is byte-identical across all three repos and dates to 21 May; the string `monthly catalog rebuild` appears in **no commit in either repo**, so the Site Refresh "Full Catalog Rebuild" button has never completed a run. The other buttons (New Products / Deals / Rush) *have* run - but each only writes its own aggregator file, never the catalog.
- **A rebuild would still not add new products to existing category pages** (`generate_content.py --skip-existing` skips them; `prune-removed-skus.ts` only removes - confirmed in the code, not the comment). Quoted to Patrick as separate work.

**Verification:** `pnpm typecheck` clean. All three repos confirmed identical afterwards (same seven counts, `skuFilterMode: full`, CTA rule no longer firing, `rank-matches.ts` present). **Not committed, not pushed, not deployed** - the seven pages are baked at build time, so nothing changes on the live site until the next deploy. `/cat/pens` is a prebuilt root path, so it goes live the moment the deploy finishes (no warmup needed).

**Deploy gates (Ali):** (1) `/cat/pens` shows a 365-product grid, not the lead form; (2) `/cat/office` (1,738) paginates and filters sanely - the largest of the seven; (3) in Studio, open a Category Override and type `pens` - **Pens** should be the first result; (4) type a bare SKU in a product picker and confirm the exact match leads.

### [x] FIX-820 (part 1 of 3): Name Brand Writing into `/cat/pens` + expand-safe wired into the rebuild - CODE COMPLETE 2026-08-11 (staged, NOT committed)

Three follow-ups were batched onto one deploy. **Parts 1 and 2 below are done; part 3 (the Merchant Listings / Product Snippets fields) is deliberately NOT started** - its blocking piece is the return-policy wording, which Patrick has been asked about, and the same question covers image licensing. Both stay untouched until he answers.

**Context correction worth recording, because it was nearly diagnosed as a broken pipeline.** `data/geiger/products.json` is still the 21 May scrape (7,957 products, md5 `a65d93f4...`, byte-identical across all three repos). `git log --all -- data/geiger/products.json` in the **deployed** repo (`patrick-perfectimprints/perfectimprints`, which is the source of truth - PRs auto-merge there and it runs the live site) returns **exactly one commit, the initial commit**. Nothing is stuck: the Site Refresh aggregator buttons each auto-merge their own PR and those ARE landing (`chore(deals)` 08-03, `chore(new-products)` 07-19 / 07-08 / 07-06, `chore(rush-products)` 07-06), but each writes only its own aggregator file. The **Full Catalog Rebuild has simply never been run** - it is an hours-long job. So the six other allowlisted categories necessarily report `no change`, and the pens numbers below are against the May catalog.

**Part 1 - Name Brand Writing folded into `/cat/pens`.** Patrick asked for Geiger's separate name-brand shelf by name. `Home > Writing Instruments > Name Brand Writing` is a **sibling** of the `Home > Writing Instruments > Pens` path `/cat/pens` maps to, which is why the existing derivation missed it.

- **It is a BRAND shelf, not a product type.** 36 products sit under it (not the ~83 quoted, which is presumably Geiger's live count today). **21 of the 36 already carry a Pens-subtree path**, so they were on the page already. Of the remaining 15, **6 are not pens**: `Sharpie Fine Point`, `Sharpie Mini`, `Sharpie Metallic`, `Sharpie Twin Tip` (markers), `Post-It Flag+ Highlighter`, and `uni-ball Chroma Pencil (0.5mm)`.
- **Filtered, not taken wholesale.** New optional `alsoInclude: {path, why, exclude}` on an allowlist entry, with `isNotAPen` as the predicate. Two rules, because the offenders split into two kinds and neither rule catches the other's: (1) cross-listed under `Highlighters & Markers` or `Pencils` - Geiger's own filing, the strongest signal, and the only thing that catches the four Sharpies, whose names never contain the word "marker"; (2) the NAME or `product_type_unigram` says highlighter/marker/pencil - catches the Post-it highlighter, which Geiger cross-lists nowhere. It deliberately does **not** read the description, where a real pen's copy can mention a marker in passing and cost us a real pen.
- **Result: 9 real pens added, `pens` 365 → 374, ZERO removed** (verified by set difference against `git show HEAD:data/categories/pens.json`, not by arithmetic). The 9: Paper Mate Flair, uni-ball Power Tank RT, uni-ball Vision Roller, Pilot Dr. Grip Center Of Gravity, Pilot Acroball Pure White, Pilot Acroball Colors, Bic Clic Stic Grip, Bic PrevaGuard Clic Stic, Bic PrevaGuard Round Stic.
- **A sibling rule structurally cannot remove anything.** The mapped path is tested FIRST and taken unconditionally; `exclude` only ever runs on products arriving via the sibling path. This is the one property worth preserving if the function is ever refactored.
- Diff hygiene held: 13 changed lines in `pens.json` (9 SKU insertions in catalog order + `rawSkuCount`/`filteredSkuCount`), CRLF preserved, no other category touched. `--verbose` now lists every filtered-out product by SKU and name, so the judgement call is auditable rather than silent.

**Part 2 - `expand-safe` is now a step of the rebuild.** It had to be remembered by hand, and every rebuild without it made those seven pages progressively smaller: `generate_content.py --skip-existing` skips existing category JSONs and `prune-removed-skus.ts` only ever REMOVES, so nothing in the job added products back.

- One step in [.github/workflows/monthly-rebuild.yml](.github/workflows/monthly-rebuild.yml)'s assemble job, **immediately after the prune and before the summary**. Both positions are load-bearing: after the prune because the script re-derives from the catalog and would otherwise undo a prune that ran later; before the summary so the changes reach the diff, the PR and Patrick's email. `add-paths` already covers `data/categories`, so no change there.
- Same `if: needs.scrape-ab.result == 'success'` guard as the prune - a run without Phase B has no fresh catalog, so re-deriving would be pointless.
- **Failure behaviour is deliberate and is a real change:** the step is NOT `continue-on-error`, so a crash fails the assemble job and no PR is opened or merged. Previously a rebuild would have shipped without the expansion. The script warns-and-continues on a missing category JSON, a missing mapping, or a zero-SKU resolve, so the only realistic hard failure is malformed JSON - at which point not deploying is the right outcome. **This workflow auto-merges its own PR and deploys production**, so that trade should be a conscious one.

**Deliberately NOT done:** the allowlist was not widened past the pens sibling; the other 58 `full-capped-60` roots are untouched (mapping problem, not a filter problem - see FIX-810); the CTA rule is unchanged; no Studio-facing surface changed, so `perfect-imprints-sanity-guide.html` needed no edit and there is **no webhook Filter change** (no new Sanity document type, no new read surface, no new cache tag). Nothing here touches a render path, so `/cat` staticness is unaffected.

**Deploy gates (Ali):** (1) `/cat/pens` renders a **374**-product grid, not the lead form; (2) spot-check one of the other six (e.g. `/cat/office`, 1,738) and confirm nothing off-topic appeared; (3) nothing else needed - the workflow step only matters the next time Patrick presses Full Catalog Rebuild.
