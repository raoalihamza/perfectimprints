...
- **Known limitation, reported not hidden:** if Turnstile keys ARE set on the target deployment, every scripted POST is refused as unverified - the CAPTCHA working correctly. The script detects that specific 400, records the affected checks as INFO with a loud note instead of a misleading FAIL, and pushes them onto the manual list.
