...
- **Fix 1 — word-count control.** New Studio field `blogPost.aiWordCount` ("Approximate word count", number, default 1700, validated 1200-2400) in the AI fieldset; the action sends it; the route clamps server-side. New pure module [lib/ai/word-budget.ts](lib/ai/word-budget.ts): the route no longer asks the model for "1,500-2,000 words" (unreliable — that's how 963 happened) but fixes the section count from the target (`listIdeaCount` 8-12 / `singleSectionCount` 4-6), reserves an intro budget (120-180), splits the remainder per section (`buildWordBudget`), and states ALL the numbers in the prompt. The fixed 900-word thin floor became **dynamic: 75% of target** (`THIN_FLOOR_RATIO`) → same clean 502 "click Generate again". `max_tokens` raised to 8000 for the 2400 top of range. Guide is honest that it's a target (±~15%), not an exact count.
