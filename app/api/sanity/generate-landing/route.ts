...
// rejected with a clean 502 retry) → AUTO-PLACE internal links into the body
// paragraphs (lib/ai/place-internal-links in 'page' link-shape mode — the
// landing bodies reuse the page-builder portableBody, whose link markDef is
// `{_type:'link', _key, href}` with NO openInNewTab) → build the three body
// fields as page-legal Portable Text (buildPageBody / buildPageSectionsBody —
// optionsIdeas carries its section headings IN the body as h2 blocks) → match
// one related-products strip (lib/ai/related-products, relevance-floored,
// skipped under 2 products). The Studio action patches the draft for review.
// Never publishes.
//
...
// runtime/dynamic exports below mirror the generate-blog/-video/-page routes.
...
import type { BlogBodyInput, BlogRichText } from '@/lib/portable-text/build-blog-body';
...
export async function POST(request: Request) {
  ...
  const landmarks = cleanStrings(body.landmarks);
  ...
  try {
    ...
    if (faqs.length < 3) {
      return NextResponse.json(
        {
          error: `The AI returned too few usable FAQs (${faqs.length} against the 3 to 6 target). Click Generate again to retry.`,
        },
        { status: 502 },
      );
    }
    ...
    const allParagraphs = [
      ...localIntro,
      ...optionsIdeas.flatMap((s) => s.paragraphs),
      ...whyUs,
    ];
    ...
    if (words < MIN_BODY_WORDS) {
      return NextResponse.json(
        {
          error: `The AI body came back thin (~${words} words against the ~800 to 1300 target). Click Generate again to retry.`,
        },
        { status: 502 },
      );
    }
    ...
    if (missing.length > 0) {
      return NextResponse.json(
        {
          error: `The AI left out required landmark${missing.length === 1 ? '' : 's'}: ${missing.join(', ')}. Click Generate again to retry.`,
        },
        { status: 502 },
      );
    }
    ...
    // 2) Internal links from real targets only — now including videos and OTHER
