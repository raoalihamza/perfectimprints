...
import type { NewProductsFacetSection, NewProductsFilterState } from '@/lib/new-products-filter';

interface NewProductsFilterSidebarProps {
  sections: NewProductsFacetSection[];
  state: NewProductsFilterState;
  onChange: (next: NewProductsFilterState) => void;
}

const COLLAPSED_VISIBLE = 6;

export function NewProductsFilterSidebar({
  sections,
  state,
  onChange,
}: NewProductsFilterSidebarProps) {
  ...
  const toggleValue = useCallback(
    (field: string, valueId: string) => {
      const next: NewProductsFilterState = { ...state };
...
interface FacetValueListProps {
  section: NewProductsFacetSection;
  selected: string[];
