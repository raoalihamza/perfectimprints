...
import type { RushProductsFacetSection, RushProductsFilterState } from '@/lib/rush-products-filter';

interface RushProductsFilterSidebarProps {
  sections: RushProductsFacetSection[];
  state: RushProductsFilterState;
  onChange: (next: RushProductsFilterState) => void;
}

const COLLAPSED_VISIBLE = 6;

export function RushProductsFilterSidebar({
  sections,
  state,
  onChange,
}: RushProductsFilterSidebarProps) {
  ...
  const toggleValue = useCallback(
    (field: string, valueId: string) => {
      const next: RushProductsFilterState = { ...state };
...
interface FacetValueListProps {
  section: RushProductsFacetSection;
  selected: string[];
