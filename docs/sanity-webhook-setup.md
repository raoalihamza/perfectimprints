...
| `productPage` / `customProduct` (in a product strip) | every **blog post / page / landing page / video** whose product strip references the edited doc (2026-07-11 — the strips accept `productPage`/`customProduct` references alongside SKU entries). Same `references($id)` lookup + slug-deref fallback as the category busting; per embedder it revalidates: blogPost → `/blog/<slug>` (path-only, matching the blog's own freshness model); video → the `videos` tag + `/videos` + `/videos/<slug>`; landingPage → `landing:<slug>` + `/<slug>`; page → `page:<slug>` + `/services/<slug>` + `/<slug>`. Needs `_id` in the projection for customProduct (no slug); productPage falls back to a slug-deref match until then |
