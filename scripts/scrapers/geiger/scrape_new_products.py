"""Weekly scrape of Geiger's New Products page.

Geiger refreshes its "New Products" feed often (items rotate in/out as products
launch and age out of the "new" window), so we can't rely on the monthly Phase B
rebuild for the dedicated `/new-products` aggregator on PI. This script hits
Searchspring directly for `Home > Shop By > New Products`, captures the products
+ the per-facet-value SKU memberships, and writes `data/geiger/new-products.json`.

Two API calls drive the data:
  1. meta.json — facet field labels (Color, Brand, Material, etc.)
  2. category.json with `bgfilter.category_path=Home > Shop By > New Products` —
     the new-product list + an embedded `facets` array with values & counts but
     NOT SKU lists.
...
returns the blue new-product SKUs).

Mirrors `scrape_deals.py` field-for-field; the only differences are the source
category path, the output filename, and a slightly bigger value-call budget
(~277 deals vs ~11 deals at time of writing, so a few more facet values).

Output: data/geiger/new-products.json.
Run:    python -m scripts.scrapers.geiger.scrape_new_products
"""
...
NEW_PRODUCTS_CATEGORY_PATH = "Home > Shop By > New Products"
...
def _fetch_base_new_products(client: ScraperClient) -> dict[str, Any]:
    page = 1
    ...
    while True:
        params = {
            ...
            "bgfilter.category_path": NEW_PRODUCTS_CATEGORY_PATH,
...
def run() -> None:
    print(f"Scraping Geiger new products from {NEW_PRODUCTS_CATEGORY_PATH!r}")

    with ScraperClient() as client:
        print("  Step 1/3: fetching meta.json for facet labels...")
        meta_facets = _fetch_meta(client)

        print("  Step 2/3: fetching base new-products + facets...")
        base = _fetch_base_new_products(client)
    ...
    output = {
        ...
        "totalNewProducts": len(normalized_products),
        "products": normalized_products,
        "facets": sections,
    }

    output_path = OUTPUT_DIR / "new-products.json"
    with open(output_path, "wb") as f:
        f.write(orjson.dumps(output, option=orjson.OPT_INDENT_2))
    print(
        f"\nWrote {len(normalized_products)} new products + {len(sections)} facet sections to {output_path}"
    )
